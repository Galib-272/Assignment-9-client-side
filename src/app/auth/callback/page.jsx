        ...
        if (targetUser && targetUser.email) {
          ...
          if (result && result.success) {
            const intendedRedirect =
              sessionStorage.getItem("auth-redirect") || "/my-ideas";
            sessionStorage.removeItem("auth-redirect");

            router.push(intendedRedirect);
          } else {
            console.error(
              "AppContext loginWithGoogle rejected the transaction.",
            );
          ...
          console.error(
            "Better-Auth did not provide a recognizable user profile structure.",
            response,
          );
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900 flex flex-col items-center justify-center gap-3 text-gray-500 text-sm transition-colors duration-300">
      ...
      <p className="font-medium animate-pulse">
        Synchronizing clearance profile...
      </p>
    </div>
  );
}
